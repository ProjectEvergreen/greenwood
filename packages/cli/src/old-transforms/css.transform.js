...
module.exports = filterStyles = async (ctx, userWorkspace) => {
  return new Promise(async (resolve, reject) => {
    try {
      if (ctx.url.indexOf('.css') >= 0) {
        const destHeader = ctx.header['sec-fetch-dest'];
        const cssPath = ctx.url.indexOf('/node_modules') >= 0
          ? path.join(process.cwd(), ctx.url)
          : path.join(userWorkspace, ctx.url);
