...
function reportStaticPathRouteCollisions(compilation) {
  const { userWorkspace } = compilation.context;
  const staticPages = getStaticPages(compilation);
  const getSourceLabel = (page) =>
    page.pageHref ? page.pageHref.replace(userWorkspace.href, "") : page.route;
  ...
  for (const page of staticPages) {
    ...
    for (const staticPath of staticPaths) {
      ...
      if (claimedBy) {
        console.warn(
          `Duplicate route detected: the static path ${staticRoute} from ${getSourceLabel(page)} is already generated by ${getSourceLabel(claimedBy)}, and will be skipped.  Please remove one of these pages, or change the value returned by getStaticPaths.`,
        );
      }
    }
  }
...
const generateGraph = async (compilation) => {
  ...
  reportStaticPathRouteCollisions(compilation);
