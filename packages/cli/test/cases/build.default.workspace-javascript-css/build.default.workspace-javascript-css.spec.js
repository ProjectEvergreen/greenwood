  ...
  describe(LABEL, function () {
    ...
    describe('<script type="module" src="..."></script> tags in the <head>', function () {
      ...
      it("should have the expected number of bundled .js files in the output directory", async function () {
        expect(
          await glob.promise(path.join(this.context.publicDir, "*.*[a-zA-Z0-9].js")),
        ).to.have.lengthOf(3);
      });

      it("should have the expected other.js file in the output directory", async function () {
        expect(
          await glob.promise(path.join(this.context.publicDir, "other.*[a-zA-Z0-9].js")),
        ).to.have.lengthOf(1);
      });
    });
    ...
    describe("<script ...>...</script> tag in the <head> with mixed attribute ordering", function () {
      it("should have two <script> tag with inline script in the <head>", function () {
        const scriptTagInline = Array.from(
      ...
      it("should have the expected inline content from inline <script> tag one in index.html", async function () {
        const scriptTagSrcOne = Array.from(
          dom.window.document.querySelectorAll("head > script:not([src])"),
        ).filter((tag) => !tag.getAttribute("data-gwd"))[0];

        expect(scriptTagSrcOne.textContent).to.contain(
      ...
      it("should have the expected inline content from inline <script> tag two in index.html", async function () {
        const scriptTagSrcTwo = Array.from(
          dom.window.document.querySelectorAll("head > script:not([src])"),
        ).filter((tag) => !tag.getAttribute("data-gwd"))[1];

        expect(scriptTagSrcTwo.textContent).to.contain(
      ...
      it("should have the expected inline content from inline <script> tag three in index.html", async function () {
        const scriptTagSrcTwo = Array.from(
          dom.window.document.querySelectorAll("head > script:not([src])"),
        ).filter((tag) => !tag.getAttribute("data-gwd"))[2];

        expect(scriptTagSrcTwo.textContent).to.contain(
          'document.getElementsByClassName("output-script-inline-three")[0].innerHTML="script tag module inline three"',
        );
      });

      it("should have the expected <script> tag of type application/json", async function () {
        const scriptJson = Array.from(
          dom.window.document.querySelectorAll('head > script[type="application/json"]'),
        );
    ...
    describe('non module <script src="..."></script> tag in the <head>', function () {
      ...
      it("should have the expected non-module.js file in the output directory", async function () {
        expect(
          await glob.promise(path.join(this.context.publicDir, "non-module.*[a-z0-9].js")),
        ).to.have.lengthOf(1);
      });
    });
    ...
    describe('popup <script src="..."></script> tag in the <body>', function () {
      ...
      it("should have the expected popup.js file in the output directory", async function () {
        expect(
          await glob.promise(path.join(this.context.publicDir, "popup.*[a-z0-9].js")),
        ).to.have.lengthOf(1);
      });
    });
