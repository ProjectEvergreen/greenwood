...
> _In the above screenshot, we can also see a demonstration of custom rendering using LitSSR with the `<simple-greeting>` component._

## Interpolate Frontmatter
At the risk of implementing / using a custom templating system (e.g. handlebars, nunjucks, etc), which is something Greenwood strives to avoid, but still recognizing that requiring JavaScript though our _graph.json_ for static sites can be a bit cumbersome.  With this new feature, which is a flag that can be enabled in your _greenwood.config.js_, frontmatter in your markdown will be available in your HTML or markdown.  Great for `<meta>` tags!
